...
Для подключения необходимо сформировать GET запрос к серверу рекламной сети и разобрать ответ согласно данной спецификации.
...
Рекламная сеть Plus1 позволяет размещать текстовые и графические баннеры, а так же их
комбинации по стандартам [MMA](http://www.mmaglobal.com/mobileadvertising.pdf).

В текстовом объявлении всегда присутствует кликабельный заголовок (длиной до 35 символов), а так же
может присутствовать дополнительный поясняющий текст(длиной до 35 символов).

Графический баннер доступен в формате GIF (в том числе анимированный), JPG, PNG.
...
- **proxy**, веб-сервис для интерграции server-to-server с рекламной сетью Plus1, форматы ответа: json, xml, html
...
* *pageId* - идентификатор показа на странице, строка [0-9a-f]{40} В рамках одного pageId ротатор будет пытаться найти разные баннеры.
...
Дополнительно можно передать заголовок *x-preferred-locale* - язык пользователя. Строка.
