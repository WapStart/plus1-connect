...
Запрос к серверу выдачи объявлений идет через хост: **ro.plus1.wapstart.ru**
...
Только для формата ответа *html* существует дополнительный необязательный параметр **markup** - тип разметки (2 - wml, 3 - xhtml, по умолчанию 3).
...
В ответе в формате xml есть нода с именем cookieSetterUrl. В ее содержимом находится картинка, которую надо "обязать" загрузить пользователя(достаточно вывести ее в html).
